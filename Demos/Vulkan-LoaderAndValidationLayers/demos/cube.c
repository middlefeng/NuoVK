...
static void demo_prepare_vs(struct demo *demo) {
    FILE* file = fopen("/Users/middleware/UserData/Source/NuoVK/Demos/Vulkan-LoaderAndValidationLayers/demos/cube-vert.spv", "r");
...
#include "cube.vert.inc"
    ...
    uint32_t* vs_code = malloc(pos);
    rewind(file);
    error = fread((void*)vs_code, pos, 1, file);
    fclose(file);
    
    demo->vert_shader_module = demo_prepare_shader_module(demo, vs_code, pos);//sizeof(vs_code));
}
...
#include "cube.frag.inc"
    ...
    FILE* file = fopen("/Users/middleware/UserData/Source/NuoVK/Demos/Vulkan-LoaderAndValidationLayers/demos/cube-frag.spv", "r");
    int error = fseek(file, 0, SEEK_END);
    size_t pos = ftell(file);
    ...
    uint32_t* fs_code = malloc(pos);
    rewind(file);
    error = fread((void*)fs_code, pos, 1, file);
    fclose(file);
    fread((void*)fs_code, pos, 1, file);
    
    demo->frag_shader_module = demo_prepare_shader_module(demo, fs_code, pos);// sizeof(fs_code));
}
...
static void demo_init_vk(struct demo *demo) {
    ...
    if (gpu_count > 0) {
        ...
        for (size_t i = 0; i < gpu_count; ++i)
        {
            ...
            vkGetPhysicalDeviceQueueFamilyProperties(physical_devices[i], &queuePropCount, NULL);
            VkQueueFamilyProperties* queueProps = malloc(sizeof(VkQueueFamilyProperties) * queuePropCount);
            vkGetPhysicalDeviceQueueFamilyProperties(physical_devices[i], &queuePropCount, queueProps);
            for (size_t i = 0; i < queuePropCount; ++i)
